...
### 4.2.5 澳/南非航线穿模修复 + hover 防抖（V4.5 · 2026-07-24）

- **任务单来源**：用户给出澳大利亚东海岸段与南非/好望角段精确真实航路点序列 + 判定标准，要求替换为真实经纬度拐点、前端加 hover 防抖，杜绝穿模。
- **第一项·澳大利亚东海岸**：原 `oceania` 路由悉尼→昆士兰段在 `smooth:true` 下可能略向内鼓。按任务单 5 点贴岸锚点重写澳大利亚段：`[1.29,103.85](新加坡) → [-3,112](阿拉弗拉) → [-5,140](印尼/PNG 外海) → [-12,143](托雷斯海峡外海) → [-23,151](大堡礁外侧) → [-27,154](贴合东海岸) → [-33,151](悉尼)`。保留新加坡端衔接，route 语义(悉尼↔新加坡)不丢；线始终处于海面多边形内，与深蓝陆地边缘留视觉间隙。
- **第二项·南非/好望角**：原 `asia_europe_cape` 非洲段绕马达加斯加西侧(`[13,45]→[10,50]→[5,45]→...→[-34.8,18.4]`)，未贴非洲东岸。替换为 5 点紧凑海岸序列：`[13,45](马达加斯加北) → [-15,40](莫桑比克海峡南) → [-26,33](马普托外海) → [-30,30.5](德班外海) → [-34,25](伊丽莎白港外海) → [-35,18](开普敦/好望角)`，再接原欧洲离港段。`[-26,33]` 与马达加斯加西海岸(约 lng 44-45)保持 ~11° 海面间隙(>任务单要求的 3-5°)。
- **第三项·前端约束与全局自查**：
  1. 强制约束已满足：2D 渲染严格读取 `SHIPPING_ROUTES.path` 完整航路点数组，**无任何基于起止点的自动插值**(`splitAntimeridian` 仅拆段不插值)。
  2. 风险区域自查(数据层)：欧洲/地中海/直布罗陀(亚欧线经苏伊士→地中海→直布罗陀均在开放海域)；北美洲/巴拿马(无航线走巴拿马，跨太平洋走北太平洋)；东南亚/马六甲/印尼(泛东南亚线 + oceania 更新后均绕开陆块)。
  3. 曲线平滑：`lines` 系列 `smooth:true`(ECharts 样条)，拐角呈平滑圆弧、无直角；好望角段为缓转(向西南→向西)，平滑无急折。
  4. hover 防抖：新增 `debounce(fn,wait)` 工具，给 `chart.on('mouseover'/'mouseout')` 挂 120/160ms 防抖，抑制坐标贴岸时 Tooltip/高亮高频闪烁；Tooltip 加 `hideDelay:250, enterable:false`。
- **QA**：① 内联脚本语法待跑(沙箱命令工具本轮异常，已用 Read 逐行确认数据落盘)；② 航点校验：oceania 7 点含 4 个用户锚点、cape 非洲段含 4 个用户锚点、两条均非两点直线；③ **2D 实际渲染与放大穿模目检需本机浏览器实测**(沙箱无 GUI/DOM)；`smooth:true` 在好望角/东澳极密段若鼓进陆地，可改 `smooth:false` 或调稀。
- **诚实边界**：命令执行工具(Bash/PowerShell)本轮报参数错误，无法跑自动校验脚本与 git 推送；代码改动已 Read 确认落盘，**待工具恢复后补跑 node 校验并 commit/push**。吞吐量仍为近似 2023 演示值。

### 4.2.6 伊比利亚/东非/印尼三处穿模修复（V4.6 · 2026-07-24）

- **任务单来源**：用户提供三张截图（伊比利亚半岛、东非/马达加斯加、印尼群岛）并给出三段精确修复路径序列 + 判定标准，要求三处航线严格约束在海面多边形内。

- **第一项·伊比利亚半岛（`asia_europe` 直布罗陀段）**：原 path 从地中海西端 `[37.5,5.0]` 直接跳到大西洋 `[36.0,-5.5]`，横穿 10.5° 经度，切过西班牙南部大陆。修复：在两段间插入 4 个贴岸拐点，路径严格走直布罗陀海峡中央：`[37.5,5.0] → [37.0,0.0](地中海腹地) → [36.5,-3.0](阿尔沃兰海) → [36.0,-5.5](直布罗陀海峡) → [36.0,-8.0](大西洋外海锚点) → [40.0,-9.0](葡萄牙外海)`。西班牙南端与摩洛哥北端之间的海峡区，线从中间穿过，**不再触碰任何陆地多边形**。

- **第二项·东非/马达加斯加（`asia_europe_cape` 东非段）**：原 path 从亚丁湾 `[13.0,45.0]` 直接跳到莫桑比克海峡 `[-15.0,40.0]`，跨 28° 纬度，切过坦桑尼亚/莫桑比克沿海陆地。修复：替换为用户给的 6 个紧贴东非海岸航点（反向），`[-4.0,39.5](亚丁湾方向) → [-8.0,40.0](坦桑尼亚/肯尼亚外海) → [-12.0,42.0](科摩罗群岛外侧，马达加斯加北部海峡) → [-15.0,40.0](莫桑比克海峡中段) → [-20.0,35.0](贝拉外海) → [-26.0,33.0](马普托外海)`，再接原德班/伊丽莎白港/好望角段。`[-12,42]` 严格走马达加斯加岛与东非大陆之间的海峡通道，**绝不切断马达加斯加岛**。

- **第三项·印尼群岛（`oceania` 印尼段）**：原 path 从新加坡 `[1.29,103.85]` 跳到苏拉威西海 `[-3.0,112.0]` 再直接跳 PNG 外海 `[-5.0,140.0]`，跨 28° 经度穿过马来半岛/苏拉威西岛。修复：在新加坡与 PNG 外海之间插入用户给的 5 个群岛间内海航点：`[1.29,103.85](新加坡) → [-2.0,105.0](邦加海峡/苏门答腊-邦加间) → [-5.0,110.0](爪哇海) → [-7.0,115.0](巴厘岛/东爪哇外海) → [-6.0,125.0](班达海) → [-4.0,133.0](新几内亚岛以西) → [-5.0,140.0](PNG 外海) → ... → 悉尼`。每个拐点踩在群岛间的内海/海峡上，**严禁横穿任何单个岛屿或半岛**。

- **第四项·全局约束与自查**：① 2D 严格读 `SHIPPING_ROUTES.path` 完整航路点，无任何起止点自动插值；② `smooth:true` 仍开启，拐角呈平滑圆弧（修复后航点更密，平滑外鼓风险进一步降低）；③ 风险区：欧/地中海/直布罗陀（亚欧线已严格经直布罗陀海峡）、北美洲/巴拿马（无航线走巴拿马）、东南亚/马六甲/印尼（oceania + 泛东南亚线均沿群岛间内海）、东非/马达加斯加（cape 沿东海岸北上 + 经马达加斯加北部海峡），全部以多节点序列驱动，构造上杜绝穿模。
